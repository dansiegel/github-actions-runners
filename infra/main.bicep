...
param githubOrganization string = 'AvantiPoint'

@description('GitHub runner group. The GitHub App installation must be allowed to manage it.')
param runnerGroup string = 'default'

@description('Runner scale set name and workflow runs-on label.')
param runnerScaleSetName string = 'avp-linux'
...
@description('Regular is reliable capacity. Spot is cheaper but jobs can be evicted.')
@allowed([
  'Regular'
  'Spot'
])
...
module runnerInfra 'resources.bicep' = {
  ...
  params: {
    ...
    runnerVmPriority: runnerVmPriority
...
output RUNNER_SCALE_SET_NAME string = runnerScaleSetName
output RUNNER_SUBNET_ID string = runnerInfra.outputs.runnerSubnetId
output RUNNER_MAX_CAPACITY int = maxRunners
